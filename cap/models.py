...
class Ocorrencia(models.Model):
    ...
    nome = models.CharField(max_length=100, help_text='Informe o nome do paciente', blank=True, null=True) # 10
    data_nascimento = models.DateField() # 11
    gestante = models.CharField(max_length=100, choices=CH_GESTANTE, blank=True, null=True)  # 14
    raca_cor = models.CharField(max_length=100, choices=CH_RACA_COR, blank=True, null=True) # 15
    municipio_residencia = models.CharField(max_length=100, help_text='Informe seu endereço', blank=True, null=True) # 20
    bairro = models.CharField(max_length=100, help_text='Informe o bairro', blank=True, null=True) # 22
    logradouro = models.CharField(max_length=100, help_text='Informe a rua/avenida', blank=True, null=True) # 23
    numero_residencia = models.PositiveIntegerField(verbose_name='Número', blank=True, null=True)  # 24
    municipio_ocorrencia = models.CharField(max_length=100, help_text='Informe onde houve a ocorrencia', verbose_name='Município da ocorrência', blank=True, null=True) # 40
    bairro_ocorrencia = models.CharField(max_length=100, help_text='Informe o bairro', blank=True, null=True) # 43
    logradouro_ocorrencia = models.CharField(max_length=100, help_text='Informe a rua/avenida', blank=True, null=True) # 44
    numero_ocorrencia = models.PositiveIntegerField(verbose_name='Número', blank=True, null=True) # 45
    hora_ocorrencia = models.CharField(max_length=5, help_text='(00:00 - 23:59)', blank=True, null=True) # 51
    ...
    motivo_violencia = MultiSelectField(max_length=200, verbose_name='Essa violencia foi motivada por', choices=CH_MOTIVO) # 55
    motivo_violencia_outros = models.CharField(max_length=200, verbose_name='Outros', blank=True, null=True) # 55 complemento
    tipo_violencia = MultiSelectField(max_length=100, help_text='Informe o tipo de violência', choices=CH_TIPO_VIOLENCIA) # 56
    meio_agressao = MultiSelectField(max_length=200, verbose_name='Meio de agressão', choices=CH_MEIO_AGRESSAO) # 57
    meio_agressao_outros =  models.CharField(max_length=200, verbose_name='Outros', blank=True, null=True) # 57 complemento
    tipo_violencia_sexual = MultiSelectField(max_length=200, choices=CH_TIPO_VIOLENCIA_SEXUAL, verbose_name='Se ocorreu violência sexual, qual o tipo?') # 58
    tipo_violencia_sexual_outros = models.CharField(max_length=200, verbose_name='Outros', blank=True, null=True) # 58 complemento
    procedimento_realizado = MultiSelectField(max_length=100, choices=CH_PROCEDIMENTO) # 59
